...
func _stamp_hole_expressions(hole_fid: String, htype: String, nominal: float) -> void:
	...
	if htype == "hex":
		...
		# Wave 6.2: circular holes track nominal + hole_compensation on regen.
		params["nominal"] = nominal
	# Prefer no-regen so this stamp does not create a second undo step on top
	# of graph_add_hole (one Undo must remove the hole entirely).
	if view.doc.has_method("graph_set_params_no_regen"):
